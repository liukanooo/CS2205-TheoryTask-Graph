...
Context {G V E: Type} 
        {pg: Graph G V E} 
        {gv: GValid G}
        (weight: G -> V -> V -> option nat)
        {A: Type}
        {path: @Path G V E pg gv A}.

Local Open Scope nat.

Fixpoint vpath_weight (g: G) (p: list V): option nat :=
  match p with
  | nil => Some 0
  | u :: p' => 
      match p' with
      | nil => Some 0
      | v :: _ => 
          option_plus (weight g u v) (vpath_weight g p')
      end
  end.

Definition valid_vpath_in_vset (g: G) (u: V) (p: list V) (v: V) (vset: V -> Prop): Prop := 
 exists P: A, (forall w, In w p -> vset w) /\ path_valid g P /\ vertex_in_path P = p /\ hd_error p = Some u /\ nth_error p (length p - 1) = Some v.

Definition min_weight_vpath (g: G) (u: V) (p: list V) (v: V): Prop :=
  valid_vpath g u p v /\
  forall p', valid_vpath g u p' v -> option_le (vpath_weight g p) (vpath_weight g p').

Definition min_weight_vpath_in_vset (g: G) (u: V) (p: list V) (v: V) (vset: V -> Prop): Prop :=
  valid_vpath_in_vset g u p v vset /\
  forall p', valid_vpath_in_vset g u p' v vset -> option_le (vpath_weight g p) (vpath_weight g p').

Definition min_weight_distance (g: G) (u: V) (v: V): nat -> Prop :=
  fun n => exists p, min_weight_vpath g u p v /\ vpath_weight g p = Some n.

Definition min_weight_distance_in_vset (g: G) (u: V) (v: V) (vset: V -> Prop): nat -> Prop :=
  fun n => exists p, min_weight_vpath_in_vset g u p v vset /\ vpath_weight g p = Some n.
